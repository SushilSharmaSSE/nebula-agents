...
Stop immediately if any feature in `BUILD_SCOPE` fails G4.6 candidate validation and the cause is not addressable in this build, if tracker validation fails and cannot be auto-repaired, if a prior approved manifest cannot be patched to `superseded`, if two approved manifests are detected for the same feature post-B4, if `INSUFFICIENT_CONTEXT` occurs for any feature in scope, or if `validate.py` or `--check-drift` fails after one repair cycle.
...
- per-feature manifest disagrees with `STATUS.md` current verdicts → fix the feature (re-run G4.5) before continuing the build
